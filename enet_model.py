...
def train_test_split(data):
    cutoff = math.floor(len(data) * 0.2)
    train = data[data.index < cutoff].copy()
    print(len(train.index))
    test = data[data.index >= cutoff].copy()
    #train = data[(data.season >= 2018) & (data.season < 2019)].copy()
    # train = data[data.season < 2015].copy()
    # test = data[data.season >= 2015].copy()
    return train, test 
...
if __name__ == "__main__":
    ...
    test_x = test.drop(["movl", 'away_ml','season'], axis=1)
    train_y = train[["movl"]]
    test_y = test[["movl"]]
    ...
    with mlflow.start_run():
        ...
        pred_df = pd.concat([test_y.reset_index(drop=True), 
                    ...
                    test['season'].reset_index(drop=True)], axis=1)
        ...
        acc = accuracy_score(pred_df['home_win'], pred_df['enet_home_win'])
        pred_df.to_csv('enet_predictions.csv', header=True, index=False)
        print(f'Accuracy: {acc}')
        mlflow.log_metric("Accuracy", acc)
        confusion_mat = confusion_matrix(pred_df['home_win'], pred_df['enet_home_win'])
        print(confusion_mat)
        f1 = f1_score(pred_df['home_win'], pred_df['enet_home_win'])
        print(f'F1 Score: {f1}')
        tracking_url_type_store = urlparse(mlflow.get_tracking_uri()).scheme
            ...
            mlflow.sklearn.log_model(en, "model", registered_model_name="ElasticnetModel")
        else:
